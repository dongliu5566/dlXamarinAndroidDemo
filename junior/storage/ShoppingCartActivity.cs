...
namespace storage
{
    ...
    public class ShoppingCartActivity : Activity
    {
        ...
        private void readSharedPreferences()
        {
            ...
            Dictionary<String, Object> mapParam = (Dictionary < String, Object> )shared.All;
            ...
            foreach (KeyValuePair<string, Object> item_map in mapParam)
            {
                ...
                if ( value is string) { // 如果配对值的类型为字符串
                    desc = String.Format("{0}\n　{1}的取值为{1}", desc, key,
                       // shared.getString(key, ""));
                       shared.GetString(key, ""));
            } else if (value is int) { // 如果配对值的类型为整型数
                desc = String.Format("{0}\n　{1}的取值为{2}", desc, key,
                        shared.GetInt(key, 2));
            } else if (value is float) { // 如果配对值的类型为浮点数
                desc = String.Format("{0}\n　{1}的取值为{2}", desc, key,
                        shared.GetFloat(key, 0.0f));
            } else if (value is bool) { // 如果配对值的类型为布尔数
                desc = String.Format("{0}\n　{1}的取值为%b", desc, key,
                        shared.GetBoolean(key, false));
            } else if (value is long) { // 如果配对值的类型为长整型
                desc = String.Format("{0}\n　%s的取值为%d", desc, key,
